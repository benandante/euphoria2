...
function sendUpdateAmount(id, d, value, purchaseId, rowNumber) {
	$.ajax({
		...
		success : function(data) {
			...
			undoStack.push(new Array(UNDO_UPDATE_USAGE_AMOUNT, purchaseId,
					rowNumber, data.foodID, data.date,
					userFoodUsageData[rowNumber][1]));
...
function sendWaste(id, d, purchaseId, value, rowNumber) {
	$
			.ajax({
				...
				success : function(data) {
					queryUpdateFoodWaste(data.members.wasteList, rowNumber,
							purchaseId);
					console
							.log('Waste have been sent to the server successfully!');
				},
				error : function(e) {
					console
							.log(e
									+ ' : Waste changes could not be sent to the server!');
					wasteFoodOffline(purchaseId, value, rowNumber, new Date()
...
function sendUsage(id, d, purchaseId, value, rowNumber) {
	$
			.ajax({
				...
				success : function(data) {
					queryUpdateFoodUsage(data.members.usageList, rowNumber,
							purchaseId);

					console
							.log('Usage have been sent to the server successfully!');
				},
				error : function(e) {
					console
							.error(e
									+ ' : Usage changes could not be sent to the server!');
					consumeFoodOffline(purchaseId, value, rowNumber, new Date()
							.format("mm/dd/yyyy hh:mm:ss"));
				}
			});
	$('#undo').removeClass('ui-disabled');
		...
		success : function(data) {
			...
			if (toBeDeleted == 0) {
				...
				for (var j = 0; j < popupLen; j++) {
					...
					document.getElementById("shoppingListTable").deleteRow(
							deleteMe);
				}
...
function deleteUserFoodItem(id) {
	...
	if (purchaseRow != -1) {
		sendDeletion(userFoodUsageData[purchaseRow][5],
				userFoodUsageData[purchaseRow][4], id, purchaseRow);
...
function buyItemFromShoppingList(id) {
	...
	$
			.ajax({
				...
				success : function(data) {
					queryBuyFromShoppingList(data.members.purchase,
							purchaseRow, id);
					console
							.log('Item from shopping to available list sent to the server!');
	...
	for (var i = 0; i < rowCount; i++) {
		...
		if (row.id == (("row") + rowID)) {
			...
			if (filteredAmount > 0) {
				...
				tableHTML = '<td><input type="number" step="1" min="0" maxlength="5" size="5" name="amount'
						+ rowID
						+ '" id="amount'
						+ rowID
						+ '" data-mini="true" value="'
			...
			if (filteredAmount > 0) {
				...
				tableHTML += '<input type="number"  disabled="true" step="1" min="0" maxlength="5" size="5" data-mini="true" id="sliderUsage'
						+ rowID + '"';
			}
		...
		if (lastValue[0] == UNDO_UPDATE_USAGE_AMOUNT) {
			...
			$
					.ajax({
						...
						success : function(data) {
							queryUpdateFoodAmount(lastValue[1], lastValue[5],
									lastValue[2], data);
							console
									.log('Changes have been sent to the server successfully!');
							$('#amount' + lastValue[1]).val(lastValue[5]);
							$('#amount' + lastValue[1]).trigger("change");
							if (lastValue[5] > 0) {
								$('#sliderUsage' + lastValue[1]).textinput(
										'enable');
							} else {
								$('#sliderUsage' + lastValue[1]).textinput(
										'disable');
			...
			$
					.ajax({
						...
						success : function(data) {
							deleteConsumption(lastValue[1], lastValue[2],
									lastValue[3]);
							var lval = $(
									'#amount'
											+ userFoodUsageData[purchaseRow][0])
									.val();
							$('#amount' + userFoodUsageData[purchaseRow][0])
									.val(
											parseFloat(lval)
													+ parseFloat(lastValue[2]));
							$('#amount' + userFoodUsageData[purchaseRow][0])
									.trigger("change");
						},
						error : function(e) {
			...
			.focusout(
					function(event, ui) {
						...
						if ($(this).val() > 0) {
							...
							if (userFoodUsageData[row][2] > 0
									&& userFoodUsageData[row][6] == 1) { //status == bought
								//this item has been purchased before and all of it is consumed
								//add new purchase
								buyNewFood(userFoodUsageData[row][5], $(this)
										.val(), 1);
								//delete previous purchase
								sendDeletion(userFoodUsageData[row][5],
										userFoodUsageData[row][4], id, row);
							} else {
								sendUpdateAmount(userFoodUsageData[row][5],
										userFoodUsageData[row][4], $(this)
												.val(),
...
function updateWasteBubbles() {
	...
	if (newWasteNumber > 0) {
		for (var j = 1; j <= pageNumber; j++) {
			$("#wasteBtn" + j + " .ui-btn-text").text(
					"My Bin (" + newWasteNumber + ")");
			$("#wasteBtn" + j + " .ui-btn-text").parent()
					.css("color", "0B6138");
		}
	} else {
		for (var j = 1; j <= pageNumber; j++) {
			$("#wasteBtn" + j + " .ui-btn-text").text("My Bin");
			$("#wasteBtn" + j + " .ui-btn-text").parent().css("color", "white");
		}
	}
